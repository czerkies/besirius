...
if (isset($_POST['sendMail'])) {
  ...
  && isset($_POST['rbt']) && empty($_POST['rbt'])){
      ...
      if(empty($_POST['email']) || !filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)){
        $msg .= "<label for='email'>Veuillez inscrire un <b>E-mail</b>.</label><br>";
      }
      if(!empty($_POST['telephone']) && (!is_numeric($_POST['telephone']) || strlen($_POST['telephone']) < 8)){
        $msg .= "<label for='telephone'>Veuillez inscrire un <b>Téléphone</b> valide.</label><br>";
      }
      if(empty($_POST['message'])){
        $msg .= "<label for='message'>Veuillez inscrire un <b>Message</b>.</label><br>";
      }
